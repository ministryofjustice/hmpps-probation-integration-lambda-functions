...
const handler = async (event: SQSEvent, context: Context): Promise<void> => {
  ...
  const message = JSON.parse(event.Records[0].messageId) as AllocationMessage
  ...
  const allocationId = message.additionalInformation.get('allocationId') as string

  // Get token from HMPPS Auth
  const token = await hmppsAuthClient.getSystemClientToken(username)

  // Get current state of allocation
  const allocation = await hmppsWorkload.getPersonAllocation(allocationId, token)
